...
공업일 : $\delta w_t =-v\ dp$,     $w_t=-\int v\,dp$ (개방계의 일량, 펌프, 터빈, 압축기)
...
- 증기
  ...
  > 포화액-->습증기: 
  > 습증기-->건포화증기:
  > Note over 포화액,건포화증기: 0<x<1, 증발열(잠열), 끓는 점
  > 건포화증기->과포화증기:
...
1. 랭킨사이클(Rankine Cycle)
   효율 $\eta_R=\frac{(터빈일량)-(압축일량)}{보일러열량}=\frac{(h_3-h_4)-(h_2-h_1)}{h_3-h_2}$ 
   압축일(압축기의 일량)은 작은 경우 무시할 수 있다
   ![pat00002](https://user-images.githubusercontent.com/43361320/102498115-e7af8080-40bc-11eb-82bf-7bce33669236.png)
   
   | 과정                                                         | 변화                                                         |
   | ------------------------------------------------------------ | ------------------------------------------------------------ |
   | 급수펌프(1$\Rightarrow$2) <br />가역 단열 압축 (정적압축-비압축성유체) | 복수기에서 응축된 포화수를 급수펌프를 이용하여 보일러에 급수. |
   | 보일러(2$\Rightarrow$3)<br />정압가열                        | 보일러에서 열을 공급. 포화수를 거쳐 건포화증기가 됨.         |
   | 터빈(3$\Rightarrow$4)<br />단열 팽창                         | 터빈내에서 단열팽창을 통해 터빈에 기계적 에너지로 변환.      |
   | 복수기(4$\Rightarrow$1)<br />정압방열                        | 터빈을 빠져나온 습증기를 정압하에서 냉각. 포화수로 응축.     |
   
   
   
2. 재열사이클(Reheat cycle)
   효율 $\eta_{Reh}=\frac{(터빈일량)-(압축일량)}{보일러열량}=\frac{\{(h_3-h_4)+(h_5-h_6)\}-(h_2-h_1)}{(h_3-h_2)+(h_5-h_4)}$ 
   ![pat00006](https://user-images.githubusercontent.com/43361320/102500555-c7cd8c00-40bf-11eb-9e2b-c74465f5233b.png)
   개선율$\varepsilon=\frac{\eta_{Reh}-\eta_{R}}{\eta_{R}}\times100(\%)$
   
3. 재생사이클(Regenerative Cycle)
   효율 $\eta_{Reg}=\frac{(터빈일량)-(압축일량)}{보일러열량}$
                    $=\frac{\{(h_2-h_3)+(1-m_1)(h_3-h_4)+(1-m_1-m_2)(h_4-h_5)\}-\{(h_1-h_{10})+(1-m_1)(h_9-h_8)+(1-m_1-m_2)(h_7-h_6)\}}{(h_2-h_1)+(1-m_1)(h_{10}-h_9)+(1-m_1-m_2)(h_8-h_7)}$ 
   ![화면 캡처 2020-12-18 004118](https://user-images.githubusercontent.com/43361320/102509343-c86b2000-40c9-11eb-8e19-24f96aab4387.png)$m_1=(h_3-h_{10})=(1-m_1)(h_{10}-h_9)$
   $m_2(h_4-h_8)=(1-m_1-m_2)(h_8-h_7)$
   개선율$\varepsilon=\frac{\eta_{Reg}-\eta_{R}}{\eta_{R}}\times100(\%)$

## 5. 가스동력 사이클

1. 내연기관 실린더
   > T.D.C(Top Dead Center) : 상사점. 실린더가 압축되어 멈추는 지점.
   > B.D.C(Bottom Dead Center) : 하사점. 실린더가 팽창되어 멈추는 지점.
   > S(Stroke) : 행정. T.D.C 와 B.D.C 사이의 거리.
   > $v_c$ : 통극(=극간=간극=틈새=연소실)체적
   > $v_s$(stroke volume) : 행정
   > $v$ : 실린더 체적
   > $\varepsilon$(compression ratio) : 압축비 = $\frac{v}{v_c} = \frac{v_c + v_s}{v_c}$
   
2. 오토사이클(Otto Cycle)
   불꽃점화기관, 전기점화기관, 가솔린기관
   흡입 - 1 - 단열압축 - 2 - 정적가열(착화) - 3 - 단열팽창 - 4 - 정적냉각(배기) - 흡입
   $\eta_o = 1-\frac{1}{\epsilon^{k-1}}$
![unnamed](https://user-images.githubusercontent.com/43361320/102503993-a53d7200-40c3-11eb-9557-b0d29aef1218.png)
   
3. 디젤사이클(Disel Cycle)
   압축착화기관, (중,저속)디젤기관
   흡입 - 1 - 단열압축 - 2 - 정압팽창(착화) - 3 - 단열팽창 - 4 - 정적냉각(배기) - 흡입
   $\eta_d = 1-\frac{1}{\varepsilon^{k-1}}\frac{\sigma^{k}-1}{k (\sigma-1)}$
   $\sigma = \frac{v_3}{v_2}$(=체절비=단절비=cut off ratio)
   ![그림1](https://user-images.githubusercontent.com/43361320/102504253-efbeee80-40c3-11eb-9531-bcfecddcdaa7.png)

4. 사바테사이클(Sabathe Cycle)
   합성사이클(Combind Cycle), 이중연소사이클, 고속디젤기관
   흡입 - 1 - 단열압축 - 2 - 정적가열(착화) - 3 - 정압팽창 - 4 - 단열팽창 - 5 - 정적냉각(배기) - 흡입
   $\eta_d = 1-\frac{1}{\varepsilon^{k-1}}\frac{\rho\sigma^{k}-1}{(1-\rho) + k (\sigma-1)}$
   $\rho=\frac{p_4}{p_2}=\frac{p_3}{p_2}$
   ![img](https://user-images.githubusercontent.com/43361320/102505717-ac657f80-40c5-11eb-9650-effdef03a17b.jpg)

5. 오토사이클, 디젤사이클, 사바테사이클 비교
   압축비 : $\varepsilon_d>\varepsilon_s>\varepsilon_o$
